...
with st.container():
    ...
    with col1:
        st.image('images/profile-pic.png',width=220)
    with col2:
        st.title(":white[Karma Gurung]")
        st.write('Data Scientist | Statistical Analysis | Machine Learning | Data Mining/Wrangling SQL | Python | Researcher')
...
with st.container():
    selected = option_menu(
        menu_title=None,
        options=["Projects","Tableau","About Me","Contact"],
...
if selected == "Projects":
    tab1, tab2, tab3 = st.tabs(["1.:movie_camera: Movie Recommendation App","2.:chart_with_upwards_trend: Stock Prediction App","3.:house: NYC Housing Price Prediction App"])
...
if selected == "Contact":
    ...
    with st.container():
        ...
        with col1:
            #st.write("Github")
            st.markdown(github_text,unsafe_allow_html=True)
        with col2:
            #st.write("LinkedIn")
            st.markdown(linkedin_text,unsafe_allow_html=True)
